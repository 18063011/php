...
<html>
  ...
  <body>
    ...
    <div class="container mt-2">
        <form method="post" enctype="multipart/form-data">
            <div class="form-outline mb-3 w-50 m-auto">
                ...
                <input type="text" class="form-control" placeholder="Nhập tên sách" id="tensach" name="tensach" required="required" autocomplete="off">
            </div>
            <div class="form-outline mb-3 w-50 m-auto">
                <label for="tacgia" class="form-label">Tác giả</label>
                <input type="text" class="form-control" placeholder="Nhập tên tác giả" id="tacgia" name="tacgia" required="required" autocomplete="off">
            </div>
            <div class="form-outline mb-3 w-50 m-auto">
            ...
            <div class="form-outline mb-3 w-50 m-auto">
                ...
                <input type="number" class="form-control" placeholder="Nhập giá (VNĐ)" id="gia" name="gia" required="required" autocomplete="off">
            </div>
            <div class="form-outline mb-3 w-50 m-auto">
            ...
            <div class="form-outline mb-3 w-50 m-auto">
                <input type="submit" class="btn btn-primary mb-3 px-3" id="submit" name="submit" value="Thêm sản phẩm">
            </div>
        </form>
    </div>
    <?php
    if(isset($_POST['submit'])&&$_POST['submit']){
        ...
        $select_product="SELECT * FROM `sach` WHERE tensach='$tensach'";
        $select_query_product=mysqli_query($con,$select_product);
        $number=mysqli_num_rows($select_query_product);
        // Image
        $hinh_filename=$_FILES['hinh']['name'];
        $hinh_tmp_name=$_FILES['hinh']['tmp_name'];
        $hinh_error=$_FILES['hinh']['error'];
        $hinh_myextension=explode('.',$hinh_filename);
        $hinh_extension=strtolower(end($hinh_myextension));
        $allowed=array('jpg','jpeg','png');
        // Check empty and insert image
        if($tensach=='' || $tacgia=='' || $danhmuc=='' || $gia=='' || $hinh_filename==''){
          echo "<script>alert('Vui lòng nhập đầy đủ các thông tin')</script>";
          exit();
        }elseif(!in_array($hinh_extension,$allowed)){
          echo "<script>alert('Vui lòng chọn file hình (jpeg,jpg,png)')</script>";
        }elseif($hinh_error!==0){
          echo "<script>alert('Có lỗi xảy ra khi upload file')</script>";
        }elseif($number>0){
          echo "<script>alert('Sản phẩm sách này đã tồn tại')</script>";
        }else{
          // $hinh_name=time().'.'.$hinh_filename;
          move_uploaded_file($hinh_tmp_name,"img/sanpham/$hinh_filename");
          // Insert query
          $insert_product="INSERT INTO `sach` (tensach,tacgia,id_danhmuc,gia,hinh)
          VALUES ('$tensach','$tacgia','$danhmuc','$gia','$hinh_filename')";
          $result_product=mysqli_query($con,$insert_product);
          if($result_product){
            echo "<script>alert('Nhập sách thành công')</script>";
