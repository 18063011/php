...
if($number_update>0){
    while($row_update=mysqli_fetch_array($result)){
        $id_user=$row_update['id_user'];
        $name=$row_update['name'];
        $email=$row_update['email'];
        $address=$row_update['address'];
        $phone=$row_update['phone'];
        $method=$row_update['method'];
        $total_sach=$row_update['total_sach'];
        $total_gia=$row_update['total_gia'];
...
if(isset($_POST['submit'])&&$_POST['submit']){
    ...
    $status=$_POST['status'];
    $update_product="UPDATE `order` SET id_user='$id_user',name='$name',email='$email',phone='$phone',address='$address',method='$method',total_sach='$total_sach',total_gia='$total_gia',status='$status'
    WHERE id_order=$id";
    ...
    <div class="container mt-2">
        <form method="post" enctype="multipart/form-data">
            <div class="form-outline mb-3 w-50 m-auto">
                ...
                <input type="text" class="form-control" placeholder="Nhập id_user (nhập guest nếu là khách vãng lai)" 
                name="id_user" required="required" autocomplete="off"
            ...
            <div class="form-outline mb-3 w-50 m-auto">
                ...
                name="phone" required="required" autocomplete="off"
                value="<?php echo $phone; ?>">
            </div>
            <div class="form-outline mb-3 w-50 m-auto">
            <label for="method" class="form-label">Phương thức thanh toán</label>
                <select name="method" class="form-select">
                  <option value="" disabled selected><?php echo $method; ?></option>
            ...
            <div class="form-outline mb-3 w-50 m-auto">
                ...
                <input type="text" class="form-control" placeholder="Tên sản phẩm (Giá x Số lượng)" 
                name="total_sach" required="required" autocomplete="off"
                value="<?php echo $total_sach; ?>">
            </div>
            <div class="form-outline mb-3 w-50 m-auto">
                <label for="total_gia" class="form-label">Tổng tiền đơn hàng</label>
                <input type="number" class="form-control" placeholder="Nhập tổng tiền của đơn hàng" 
                name="total_gia" required="required" autocomplete="off"
                value="<?php echo $total_gia; ?>">
            </div>
            <div class="form-outline mb-3 w-50 m-auto">
                <label for="status" class="form-label">Tình trạng đơn hàng</label>
                <input type="text" class="form-control" placeholder="Nhập tình trạng đơn hàng" 
                name="status" required="required" autocomplete="off"
                value="<?php echo $status; ?>">
            </div>
            <div class="form-outline mb-3 w-50 m-auto">
                <input type="submit" class="btn btn-primary mb-3 px-3" id="submit" name="submit" value="Sửa đơn hàng">
            </div>
        </form>
    </div>
