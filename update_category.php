...
if($number_update>0){
    while($row_update=mysqli_fetch_array($result)){
        $id_danhmuc=$row_update['id_sach'];
        $tendanhmuc=$row_update['tendanhmuc'];
    }
}
...
if(isset($_POST['submit'])&&$_POST['submit']){
    ...
    $select_query="SELECT * FROM `danhmuc` WHERE tendanhmuc='$tendanhmuc'";
    $result_select= mysqli_query($con,$select_query);
    $number=mysqli_num_rows($result_select);
    if($number>0){
      echo "<script>alert('Danh mục này đã tồn tại trong database')</script>";
    }else{
      $update_query="UPDATE `danhmuc` SET id_danhmuc=$id,tendanhmuc='$tendanhmuc'
      WHERE id_danhmuc=$id";
      $result_update=mysqli_query($con,$update_query);
      if($result_update){
        echo "<script>alert('Danh mục được sửa thành công')</script>";
  ...
  <body>
    ...
    <form action="" method="POST" enctype="multipart/form-data" class="mb-2">
      <div class="input-group w-90 mb-2">
        <input type="text" class="form-control" name="tendanhmuc" id="tendanhmuc" placeholder="Nhập danh mục"
        value="<?php echo $tendanhmuc; ?>">
      </div>
        <input type="submit" class="btn btn-primary" name="submit" id="submit" value="Sửa danh mục">
    </form>
